...
struct Control {
  ...
  std::string DebugString() const {
    ...
    std::vector<std::string> cmds = {
      "EMPTY", "TERMINATE", "ADD_NODE", "BARRIER", "ACK", "HEARTBEAT", "DYNAMIC_ADD_NODE"};
    std::stringstream ss;
    ss << "cmd=" << cmds[cmd];
    if (node.size()) {
      ss << ", node={";
  ...
  /** \brief all commands */
  enum Command { EMPTY, TERMINATE, ADD_NODE, BARRIER, ACK, HEARTBEAT, DYNAMIC_ADD_NODE };
  /** \brief the command */
  Command cmd;
  /** \brief node infos */
...
struct Meta {
  ...
  Meta() : head(kEmpty), app_id(kEmpty), customer_id(kEmpty),
           ...
           request(false), push(false), simple_app(false), last_pull(false) {}
  std::string DebugString() const {
    std::stringstream ss;
